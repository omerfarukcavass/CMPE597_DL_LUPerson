...
parser.add_argument('--eval_freq', default=20, type=int, metavar='N',
                    help='evaluation frequency (default: 10)')
parser.add_argument('--resume', default='', type=str, metavar='PATH',
                    help='path to latest checkpoint (default: none)')
parser.add_argument('--auto_resume', type=int, default=0)
...
parser.add_argument('--lr_mult', '--learning-rate-multiplier', default=0.1, type=float,
                    metavar='LRM', help='learning rate update multiplier', dest='lr_mult')
parser.add_argument('--schedule', default='50,100,150,200,250', type=str,
...
parser.add_argument('--aug_type', default='reid-affine', type=str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = CMDM(args.eval_path, args.eval_name, 'train', train_transformer)
    ...
    if args.auto_resume:
        ...
        if os.path.isfile(latest_checkpoint):
            ...
            if len(snaps) > 0:
                ...
                print(f'=>rank[{args.rank}] no previous snapshots exist for auto-resume.')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.rank == 0:
            print(f"{time_str}: Epoch[{epoch}]; Loss={loss:.3f}; Acc={acc1.mean().item():.3f}; " +
